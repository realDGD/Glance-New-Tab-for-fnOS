...
export const WARMUP_STATE_STORAGE_KEY = "fnos-warmup-state";
export const WARMUP_TIMEOUT_MS = 20_000;
export const MAX_NEWTAB_WARMUP_WAIT_MS = 10_000;

export class FnConnectWarmupManager {
  constructor(options = {}) {
    this.sessionStorage = options.sessionStorage ?? (typeof chrome !== "undefined" ? chrome.storage?.session : null);
    this.tabsApi = options.tabsApi ?? (typeof chrome !== "undefined" ? chrome.tabs : null);
    this.markWarmedFn = options.markWarmedFn ?? null;
    this.state = "idle";
    this.warmupId = null;
    ...
    this.rootEnteredAt = null;
    this.dockerFrameReadyAt = null;
  ...
  isWarming() {
    return this.state === "warming" && Number.isInteger(this.helperTabId);
  }

  isHelperTab(tabId) {
    return this.isWarming() && this.helperTabId === tabId;
  }

  getPendingForHelper() {
    if (!this.isWarming()) return null;
    return {
      phase: this.phase === "root" ? "root" : "bootstrap",
      recoveryKind: "docker",
      bootstrapUrl: this.bootstrapUrl,
      rootUrl: this.rootUrl,
      checkUrl: this.checkUrl,
      targetUrl: this.checkUrl.replace(/\/app\/glance\/health\/?$/, "/app/glance/"),
      startedAt: this.startedAt,
      bootstrapEnteredAt: this.startedAt,
      bootstrapCompletedAt: this.bootstrapCompletedAt,
      rootEnteredAt: this.rootEnteredAt,
      dockerFrameReadyAt: this.dockerFrameReadyAt,
      warmupId: this.warmupId,
      isWarmupHelper: true
    };
  }

  async startWarmup(settings, source = "startup", probeAuthFn = null) {
    if (this.state === "ready") {
      return { action: "already-ready" };
    }
    if (this.state === "warming" || this.isStarting) {
      if (this.startPromise) {
        await this.startPromise;
      }
      return { action: "already-warming", warmupId: this.warmupId, helperTabId: this.helperTabId };
    }
    if (!settings?.setupCompleted || !settings?.enabled || !settings?.fnosRecoveryEnabled || !isFnOsUrl(settings?.targetUrl)) {
      return { action: "not-applicable" };
    }

    this.isStarting = true;
    this.state = "warming";
    const warmupId = (typeof crypto !== "undefined" && crypto.randomUUID) ? crypto.randomUUID() : `warmup-${Date.now()}`;
    this.warmupId = warmupId;

    let bootstrapUrl, rootUrl, checkUrl;
    try {
      bootstrapUrl = inferFnOsBootstrapUrl(settings.targetUrl);
      rootUrl = inferFnOsRootUrl(settings.targetUrl);
      checkUrl = inferFnOsHealthUrl(settings.targetUrl);
    } catch {
      this.state = "idle";
      this.isStarting = false;
      return { action: "invalid-urls" };
    }

    this.startPromise = (async () => {
      let helperTab;
      try {
        if (this.tabsApi?.create) {
          helperTab = await this.tabsApi.create({
            url: bootstrapUrl,
            active: false
          });
        } else {
        ...
        this.state = "idle";
        this.isStarting = false;
        return { action: "create-tab-failed", error: error?.message };
      }
      ...
      console.debug?.(`[FNOS prewarm] startup +0ms | helper created (tab ${helperTab.id}, warmup ${warmupId})`);

      if (probeAuthFn) {
        this.startBackgroundProbing(probeAuthFn, warmupId);
      }
  ...
  async persistEnvelope() {
    if (!this.sessionStorage?.set) return;
    if (this.state === "warming") {
      ...
      await this.sessionStorage.remove?.(WARMUP_STATE_STORAGE_KEY);
    }
  }

  async handleTabUrlCommit(tabId, newUrl) {
    if (!this.isWarming() || this.helperTabId !== tabId) {
      return false;
    }
    if (isConfiguredRootOrigin({ rootUrl: this.rootUrl }, newUrl)) {
      if (this.phase !== "root") {
        this.phase = "root";
        const now = Date.now();
        this.bootstrapCompletedAt = this.bootstrapCompletedAt ?? now;
        this.rootEnteredAt = this.rootEnteredAt ?? now;
        await this.persistEnvelope();
        console.debug?.(`[FNOS prewarm] root reached +${now - this.startedAt}ms`);
      }
      return true;
    }
    return false;
  ...
  handleFrameProbeResult(tabId, result) {
    if (!this.isWarming() || this.helperTabId !== tabId) {
      return false;
    }
    if (result === "ready") {
      this.dockerFrameReadyAt = Date.now();
      console.debug?.(`[FNOS prewarm] frame ready +${Date.now() - this.startedAt}ms`);
      return true;
    }
    this.dockerFrameReadyAt = null;
    return false;
  ...
  async markReady() {
    ...
    this.state = "ready";
    const tabToClose = this.helperTabId;
    this.helperTabId = null;
    if (this.probeTimer) {
    ...
    await this.persistEnvelope();
    if (this.markWarmedFn) {
      try {
        await this.markWarmedFn();
      } catch {
        // Ignore
      }
    }
    if (Number.isInteger(tabToClose) && this.tabsApi?.remove) {
      try {
        await this.tabsApi.remove(tabToClose);
      } catch {
        // Tab may already be closed
      }
    }
    console.debug?.(`[FNOS prewarm] strong ready +${Date.now() - this.startedAt}ms | helper closed | session warmed`);
    this.notifySubscribers({ warmed: true });
  }

  async cancelWarmup(reason = "cancelled") {
    ...
    console.debug?.(`[FNOS prewarm] warmup cancelled (${reason}) +${Date.now() - this.startedAt}ms`);
    this.notifySubscribers({ warmed: false, reason });
  }
  ...
  handleTabRemoved(tabId) {
    if (this.isWarming() && this.helperTabId === tabId) {
      this.helperTabId = null;
      this.state = "failed";
      this.failureReason = "helper-tab-closed";
      if (this.probeTimer) {
        clearTimeout(this.probeTimer);
        this.probeTimer = null;
      }
      void this.persistEnvelope();
      console.debug?.(`[FNOS prewarm] helper tab closed by user/system +${Date.now() - this.startedAt}ms`);
      this.notifySubscribers({ warmed: false, reason: "helper-tab-closed" });
    }
  }
  ...
  async waitForWarmup(tabId, navigationId, settings, timeoutMs = MAX_NEWTAB_WARMUP_WAIT_MS) {
    if (this.state === "ready") {
      return { warmed: true };
    }
    if (this.state === "failed" || !this.isWarming()) {
      return { warmed: false, reason: this.failureReason || "not-warming" };
    }
    ...
    return new Promise((resolve) => {
      ...
      timer = setTimeout(() => {
        ...
        resolve({ warmed: false, reason: "timeout" });
      }, timeoutMs);
    });
  }

  async rehydrate(probeAuthFn = null) {
    if (!this.sessionStorage?.get) return;
    try {
      const data = await this.sessionStorage.get(WARMUP_STATE_STORAGE_KEY);
      const envelope = data?.[WARMUP_STATE_STORAGE_KEY];
      if (!envelope || envelope.state !== "warming" || !Number.isInteger(envelope.helperTabId)) {
        return;
      }
      ...
      if (this.tabsApi?.get) {
        try {
          const tab = await this.tabsApi.get(envelope.helperTabId);
          if (tab && tab.url && (isFnOsUrl(tab.url) || tab.url.includes("5ddd.com") || tab.url.includes("fnos.net"))) {
            tabAlive = true;
          }
        } catch {
          tabAlive = false;
      ...
      this.state = "warming";
      this.warmupId = envelope.warmupId;
      this.helperTabId = envelope.helperTabId;
      this.startedAt = envelope.startedAt || Date.now();
      this.bootstrapUrl = envelope.bootstrapUrl;
      this.rootUrl = envelope.rootUrl;
      this.checkUrl = envelope.checkUrl;
      this.phase = envelope.phase || "bootstrap";
      this.bootstrapCompletedAt = envelope.bootstrapCompletedAt;
      this.rootEnteredAt = envelope.rootEnteredAt;

      if (probeAuthFn) {
        this.startBackgroundProbing(probeAuthFn, this.warmupId);
      }
    } catch {
      // Ignore
    }
  ...
  startBackgroundProbing(probeAuthFn, targetWarmupId) {
    const tick = async () => {
      if (this.state !== "warming" || this.warmupId !== targetWarmupId) {
        return;
      }
      if (Date.now() - this.startedAt >= WARMUP_TIMEOUT_MS) {
        await this.cancelWarmup("warmup-timeout");
        return;
      }

      try {
        const bgResult = await probeAuthFn(this.checkUrl);
        const frameReady = Boolean(
          this.dockerFrameReadyAt &&
          Date.now() - this.dockerFrameReadyAt <= 5000
        );
        const signals = combineDockerProbeSignals(bgResult?.ok, frameReady);
        if (this.phase === "root" && signals.strongReady) {
          const now = Date.now();
          if (!this.readyAt) {
            this.readyAt = now;
          }
          if (now - this.readyAt >= 250) {
            const recheck = await probeAuthFn(this.checkUrl);
            if (recheck?.ok && frameReady) {
      ...
      if (this.state === "warming" && this.warmupId === targetWarmupId) {
        this.probeTimer = setTimeout(tick, 300);
      }
    };
